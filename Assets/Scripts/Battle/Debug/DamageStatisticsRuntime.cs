...
namespace BackpackHero.Battle
{
    ...
    public sealed class DamageStatisticsRuntime : MonoBehaviour
    {
        ...
        public static void RecordOvertimeAircraftExit(BattleFaction faction) =>
            Instance?.RecordOvertimeAircraftExitInternal(faction);
        private void Record(BattleFaction faction, BattleDamageSource source, float raw, float actual, bool killedFighter)
        {
            if (!source.CountsForDamageStatistics ||
                !source.HasBackpackItem || actual <= 0f) return;
            ...
            entry.Raw += Mathf.Max(0f, raw); entry.Actual += Mathf.Max(0f, actual);
            if (killedFighter)
            {
                entry.Kills++;
                GetFactionTotals(faction).EnemyAircraftKills++;
            }
            Changed?.Invoke();
        }
        private void RecordOvertimeAircraftExitInternal(BattleFaction faction)
        {
            GetFactionTotals(faction).OvertimeAircraftExits++;
            Changed?.Invoke();
        }
        private MutableFactionTotals GetFactionTotals(BattleFaction faction)
        {
            if (!factionTotals.TryGetValue(faction, out MutableFactionTotals totals))
            {
                totals = new MutableFactionTotals();
                factionTotals[faction] = totals;
            }
            return totals;
        }
        ...
        public int GetEnemyAircraftKillCount(BattleFaction faction) =>
            factionTotals.TryGetValue(faction, out MutableFactionTotals totals)
                ? totals.EnemyAircraftKills : 0;
