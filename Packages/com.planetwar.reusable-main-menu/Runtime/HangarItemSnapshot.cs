...
namespace PlanetWar.ReusableMainMenu
{
    ...
    {
        public HangarItemSnapshot(HangarItemKind kind, string name, string description, Sprite icon, Sprite lockedIcon, bool unlocked, int level, int fragments, int requiredFragments, int goldCost, float cooldown, int spawnCount, string stats, Color backgroundColor, string unlockRequirementText, HangarDetailAttribute[] detailAttributes = null, string itemId = null, int maximumLevel = 10)
        { Kind = kind; Name = name; Description = description; Icon = icon; LockedIcon = lockedIcon; Unlocked = unlocked; Level = level; Fragments = fragments; RequiredFragments = requiredFragments; GoldCost = goldCost; Cooldown = cooldown; SpawnCount = spawnCount; Stats = stats; BackgroundColor = backgroundColor; UnlockRequirementText = unlockRequirementText; DetailAttributes = detailAttributes; ItemId = itemId; MaximumLevel = maximumLevel; }
